...
Each profile owns its own company search list. The Companies workspace shows only the active profile's employers and labels each one Scanning or Paused according to that profile's assignment. A user can search Junior's existing employer catalog and add a globally available, scan-ready company; the new profile assignment starts as Scanning. Employers that are already assigned, unavailable, or missing required administrator setup cannot be added. The normal-user catalog view shows only the company name, simple availability, a plain-language explanation, and an optional public careers link—not ATS names, source slugs, collector configuration, notes, or internal identifiers. A user can pause, resume, or remove an assigned company without changing the shared employer definition or another profile's choice. Removal requires typing `REMOVE`, stops future scans only for the active profile, and preserves collected jobs, applications, history, reports, and other historical records. The Profile / Resume summary shows the same company count and links to the workspace. Technical employer records remain part of the shared catalog for later administration. When no managed profile is active, the existing legacy YAML view remains read-only for compatibility.
