...
- Companies page with target company/source summaries, card-driven filters, search, read-only company configuration table, and per-company detail pages
- in-app report viewer with copy button and focused Ctrl+A report-content selection
...
The Companies page is read-only for now. It shows the configured target companies, source types, enabled/disabled status, source details, notes, card-driven source-type/status filters, search, and per-company detail pages from `config/target-companies.yaml` without writing config changes.
