...
Settings remains the normal-user home for safe personal and product preferences. Its About page shows the installed version, release channel, user-data location, database and profile schema versions, and safe support guidance without exposing profile contents or credentials. Email setup stores credentials through the operating system rather than ordinary settings or SQLite. Scan schedule setup stores an enabled state, local start time, selected weekdays, and email-delivery choice; it shows the calculated next run and safe status from the most recent scheduled scan. On Windows, the same page can create, inspect, update, disable, or remove Junior's single `\Junior Scheduled Scan` Task Scheduler entry. The task uses normal user permissions, stores no Windows password, and starts the same shared scan service used elsewhere. It runs only while that Windows user is signed in; logged-out service operation remains a later milestone. Administration is a separate, session-scoped safety boundary for installation-wide and technical controls. Unlocking Administration requires typing `ADMIN`; this is an explicit confirmation, not a password or protection from someone who already controls the local computer. Administration unlock state is limited to the current browser session and Junior process, and restarting Junior invalidates it. The Flask session signing key is generated locally under the user-owned database runtime directory. It is never committed or stored in YAML or SQLite; deleting it invalidates existing browser sessions.
