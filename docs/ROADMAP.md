...
| Responsive Review Jobs workflow | Completed | Scan completion leads to interactive job groups; each group renders at most 20 full cards per page, Review Needed can instead show 50 collapsed summaries with current-page-only bulk selection and navigation at both ends, supporting evidence remains expandable, and static reports remain secondary read-only exports. Applying from a review group returns to that group and removes the job from unresolved review results. Potential Top Matches retain their full category count while showing decision progress. |
