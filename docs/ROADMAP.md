...
| Responsive Review Jobs workflow | Completed | Scan completion leads to interactive job groups; each group renders at most 20 full cards per page, Review Needed can instead show 50 collapsed summaries with current-page-only bulk selection and navigation at both ends, supporting evidence remains expandable, and static reports remain secondary read-only exports. |
