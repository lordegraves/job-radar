...
`operational_event_log.py` consolidates non-scan work in a structured `junior-application.log`. One operation ID correlates GUI request start, subsystem work, database activity, and final status without recording submitted form values. The active file rotates at 2 MB and retains one previous file. Scan and evaluation traces remain run-specific; retention applies the configured per-family count and a 100 MB aggregate ceiling to dated logs. `diagnostic_log_service.py` allowlists these logs plus compatible legacy and startup logs, prioritizes the application and latest-scan logs, caps listing at 20 files, and reads only the newest 200,000 bytes. The troubleshooting package uses the same allowlist, so the unified application log is included automatically. Resolved-path checks reject traversal, symlinks escaping the logs directory, arbitrary filenames, and non-log files. The copyable support summary contains version, schema, resolved user-data location, and health-card state only. The CSRF-protected data-directory action uses the active settings location to identify the owning workspace and passes it to the operating system without invoking a shell.
