...
def test_profile_activity_migration_assigns_legacy_rows_to_active_profile(
    ...
    assert len(list((tmp_path / "backups").glob("*.pre-migration-v11-v13-*.bak"))) == 1
...
def test_initialize_database_backs_up_existing_database_before_migration(
    ...
    backup_paths = list(
        backup_directory.glob(
                "job_radar.sqlite3.pre-migration-v1-v13-*.bak"
        )
    )
...
def test_initialize_database_upgrades_v010_database_without_data_loss(
    ...
    backup_paths = list(
        (tmp_path / "backups").glob(
                "synthetic-v0.1.0.sqlite3.pre-migration-v1-v13-*.bak"
        )
    )
    assert len(backup_paths) == 1
...
def test_initialize_database_rolls_back_failed_migration(
    ...
    monkeypatch.setattr(
        ...
        lambda: (
            *existing_migrations,
            (14, "synthetic failing migration", fail_after_temporary_change),
        ),
    )
    ...
    backup_paths = list(
        (tmp_path / "backups").glob(
                "synthetic-current.sqlite3.pre-migration-v14-v14-*.bak"
        )
    )
    assert len(backup_paths) == 1
