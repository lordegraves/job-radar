...
def test_installer_build_script_requires_verified_bundle_first() -> None:
    ...
    assert (
        "artifacts\\installer\\Junior-Setup-0.2.0-RC6-build-1.9.exe"
        in script_text
    )
    validation_text = (
...
def test_windows_bundle_embeds_product_version_details() -> None:
    ...
    assert (
        'StringStruct("ProductVersion", "0.2.0 - RC6 Build 1.9")'
        in version_text
    )
    assert 'StringStruct("FileVersion", "0.2.0.5")' in version_text
