...
$defaultInstaller = Join-Path (
    $projectRoot
) "artifacts\installer\Junior-Setup-0.2.0-RC6-build-1.24.exe"
...
try {
    ...
    # Windows can leave the shared pytest temp root inaccessible after another
    # process exits. Keep release validation inside Junior's disposable
    # workspace so an unrelated OS temp permission cannot block a deployment.
    & $python -m pytest -q --basetemp $pytestBaseTemp tests
